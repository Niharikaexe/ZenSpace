...
export async function createMatch(clientId: string, therapistId: string, notes: string) {
  ...
  const { data: cQ } = await (admin as any)
    .from('questionnaire_responses').select('responses').eq('client_id', clientId).maybeSingle() as { data: { responses: Record<string, unknown> } | null; error: unknown }
  ...
  if (autoMatch) {
    ...
    await Promise.allSettled([
      ...
      createNotification({
        ...
        metadata: { therapistFirstName, therapistFullName, adminMatchNote: picked[0].summary || '', category },
      }),
    ])
  } else {

...
export async function GET(request: NextRequest) {
  ...
  for (const m of matches as Array<{ id: string; client_id: string; therapist_id: string; created_at: string }>) {
    // Has the client sent any message in this match?
    ...
    const { count: nudgeCount } = await (admin as any)
      ...
      .eq('type', 'client_chat_not_started')
    ...
    await createNotification({
      userId: m.client_id,
      type: 'client_chat_not_started',
      title: `Have you said hi to ${therapistFirstName} yet?`,
      body: `Your match with ${therapistFirstName} is waiting. Even a quick hello gets the conversation going.`,
